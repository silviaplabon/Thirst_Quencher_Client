...
(loggedInUser.admin==true&&loggedInUser.role=="Administrator")&&
    ...
    return (
                        ...
                        <div className="">
                <li className=" list-group-item liStyle liAdminStyle text-white p-2 mt-1">
                    <span className="listStyle ms-2 emailAdminStyle fontShip">{user}</span>
                    <span className="listStyle ms-2 dateAdminStyle fontShip text-center">{orderTime}</span>
                    {/* <span className="listStyle ms-2 destinationAdminStyle fontShip text-center">{destination}</span> */}
                    <span className="listStyle ms-2 statusAdminStyle text-center fontShip">
                    <select name="status" onChange={handleChange} className=" ms-2 ">
                        ...
                        }
                        {
                            status == "On Going" && <> <option>On Going</option>
                                ...
                                {
                                    order.map(detail => <AdminShipmentDetail detail={detail} time={orderTime} status={status} serviceType={type} address={destination}></AdminShipmentDetail>)
                                }
